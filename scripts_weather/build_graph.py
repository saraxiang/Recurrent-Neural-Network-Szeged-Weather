...
def build_lstm_graph_with_config(config=None):
    ...
    with lstm_graph.as_default():
        ...
        val, _ = tf.nn.dynamic_rnn(cell, inputs, dtype=tf.float32, scope="szeged_rnn")
        ...
        with tf.name_scope("output_layer"):
            ...
            weight = tf.Variable(tf.truncated_normal([config.lstm_size, config.input_size]), name="szeged_weights")
            bias = tf.Variable(tf.constant(0.1, shape=[config.input_size]), name="szeged_biases")
